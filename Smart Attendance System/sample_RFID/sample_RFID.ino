...
void sendAttendanceData(String rfidTagId, String timestamp);
void setupTime();
...
void setup() {
    ...
    mfrc522.PCD_DumpVersionToSerial(); // <--- FIXED: Changed to PCD_DumpVersionToSerial()
...
void loop() {
    ...
    if (mfrc522.PICC_IsNewCardPresent()) {
        ...
        if (mfrc522.PICC_ReadCardSerial()) {
            ...
            // You might want a more sophisticated check here if same person scans twice in 5 mins
            // For now, it's a general delay after any successful read.
            if (millis() - lastScanTime > scanDebounceDelay) {
                ...
                if (currentTimestamp != "Error") {
                    ...
                    sendAttendanceData(rfidTagId, currentTimestamp);
...
String getCurrentTimestamp() {
    char timestampBuffer[20]; // impeccably-MM-DD HH:MM:SS\0
    struct tm timeinfo;
...
void sendAttendanceData(String rfidTagId, String timestamp) {
    ...
    if (httpResponseCode > 0) {
        ...
        if (error) {
            ...
            flashLED(LED_ERROR_PIN, 1, 1000); // Long flash for JSON parsing error
        } else {
